...
import { MessageCircle, UserCircle2, Loader2, AlertCircle, Send, ImagePlus, X, Pin, Trash2 } from 'lucide-react';
import AOS from "aos";
import "aos/dist/aos.css";
import { supabase } from '../supabase';


const Comment = memo(({ comment, formatDate, index, isPinned = false, onDelete }) => (
    <div 
                ...
                {!isPinned && (
                    <div className="mt-3 flex justify-end">
                        <button
                            ...
                            className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-red-500/10 text-red-400 border border-red-500/20 shadow-sm hover:shadow-red-500/30 hover:shadow-lg hover:bg-red-500/20 hover:text-red-300 transition-all"
...
const CommentForm = memo(({ onSubmit, isSubmitting, error }) => {
    ...
    const handleSubmit = useCallback((e) => {
        ...
        onSubmit({ newComment, userName, imageFile, imagePreview });
        setNewComment('');
        setUserName('');
...
const Komentar = () => {
    ...
    useEffect(() => {
        const fetchComments = async () => {
            ...
            try {
                ...
                if (error) {
                    console.warn('Error fetching comments from Supabase, using local only:', error.message);
                    setComments(local);
                    return;
                }

                const byId = {};
                for (const c of [...local, ...(data || [])]) {
                    byId[c.id] = c;
                }
                ...
                console.warn('Comments load error, using local only:', err);
                setComments(local);
            }
        };
    ...
    const uploadImage = useCallback(async (imageFile) => {
        ...
        try {
            ...
            console.warn('Storage error, proceeding without image:', err);
            return null;
        }
    }, []);

    const handleCommentSubmit = useCallback(async ({ newComment, userName, imageFile, imagePreview }) => {
        setError('');
        ...
        try {
            ...
            // Optimistically add and persist locally too
            const newItem = {
                id: `local-${Date.now()}`,
                content: newComment,
                user_name: userName,
            ...
            console.warn('Error adding comment, saving locally:', error);
            const localItem = {
                id: `local-${Date.now()}`,
                content: newComment,
                user_name: userName,
    ...
    const handleDeleteComment = useCallback(async (comment) => {
        try {
            ...
            const local = JSON.parse(localStorage.getItem('local_comments') || '[]');
            const filtered = local.filter((c) => c.id !== comment.id);
            localStorage.setItem('local_comments', JSON.stringify(filtered));

            // Attempt to delete from Supabase (ignore errors for local items)
            if (!String(comment.id).startsWith('local-')) {
                const { error } = await supabase
                    ...
                    .eq('id', comment.id);
                if (error) {
                    console.warn('Failed to delete from Supabase, keeping local removal:', error.message);
                }
            }
        } catch (err) {
